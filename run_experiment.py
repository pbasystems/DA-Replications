...
def compute_dataset_feature_stats(root_dir, fd):
    train_df = pd.read_csv(Path(root_dir) / f"train_{fd}.txt", sep=r"\s+", header=None, names=COLUMN_NAMES)
    test_df = pd.read_csv(Path(root_dir) / f"test_{fd}.txt", sep=r"\s+", header=None, names=COLUMN_NAMES)
...
def run_trial(seed, source_dataset, target_dataset, target_test_dataloader):
    ...
    model = LSTM_DANN(
        input_size=24, hidden_size=64, f_size=32, num_layers=1,
        lstm_dropout=0.5, regressor_dropout=0.3, classifier_dropout=0.3, alpha=0.8,
    ).to(DEVICE)
...
def main():
    ...
    source_dataset = CMAPSSDataset(
        root_dir=DATASET_PATH, fd=SOURCE_FD, split="train",
        window_size=30, r_early=125, feature_stats=source_stats,
    )
    target_dataset = CMAPSSDataset(
        root_dir=DATASET_PATH, fd=TARGET_FD, split="train",
        window_size=30, r_early=125, feature_stats=target_stats,
    )
    target_test_dataset = CMAPSSDataset(
        root_dir=DATASET_PATH, fd=TARGET_FD, split="test",
        window_size=30, r_early=125, feature_stats=target_stats,
    )
    ...
    for trial in range(N_TRIALS):
        metrics = run_trial(trial, source_dataset, target_dataset, target_test_dataloader)
        print(f"Trial {trial + 1}/{N_TRIALS}: RMSE={metrics['rmse']:.2f}  MAE={metrics['mae']:.2f}  Score={metrics['score']:.2f}")
